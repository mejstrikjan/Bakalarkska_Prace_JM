...
const db = require('../db/connection'); // nebo ../db/connection dle struktury
...
router.get('/reservations/:date', async (req, res) => {
  ...
  try {
    // Načteme rezervace pro daný den a jejich headsety
    ...
    // Příprava struktury pro každý headset
    const result = {};

    rows.forEach(row => {
      const headsetId = row.headsetID;
      const start = row.time.substring(0, 5); // HH:mm
